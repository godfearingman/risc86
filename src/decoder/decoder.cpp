...
decoder::to_instr(std::uint32_t instr, std::uint32_t *regs,
  ...
  switch (opcode) {
  case 0x33:
    return std::make_unique<instruction_r>(instr, regs, memory);
  case 0x03:
  case 0x13:
    return std::make_unique<instruction_i>(instr, regs, memory);
  case 0x23:
    return std::make_unique<instruction_s>(instr, regs, memory);
  }
